...
| `guards.max_feed_lag_ms: 1000` | §Z.5 | HALT new entries while the rolling MEDIAN l2Book feed lag exceeds this. MEASURED baseline in ap-northeast-1 is ~285-310ms — that is HL's own publish/consensus floor, not your network (connect RTT 1.7ms), so the halt is a FAULT threshold at ~3x baseline, NOT a budget check. 400 was tried first and sits too close to baseline to survive jitter. Measured on l2Book ONLY (trades are REPLAYED on reconnect with old stamps and give a false 150s+ spike). Median, so one GC pause cannot halt the book. Open positions keep managing their exits. A negative median = local clock ahead of the exchange (NTP), not a fast feed. |
